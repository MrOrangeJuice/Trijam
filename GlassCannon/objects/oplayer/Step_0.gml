...
key_shoot_pressed = keyboard_check_pressed(vk_lshift) || keyboard_check_pressed(ord("X")) || keyboard_check_pressed(ord("O"));
...
if (key_shoot_released)
{
	flashing = false;
	flash = 10;
	bullet = instance_create_layer(x+3,y+2,"Bullets",oBullet);
	if(image_xscale == 1)
	{
		bullet.xdir = 1;
	}
	...
	if(charge >= 3)
	{
		audio_play_sound(snd_ShootBig,5,false);	
	}
	else
	{
		audio_play_sound(snd_ShootSmall,5,false);	
	}
